...
def login():
    ...
    for _ in range(3):
        try:
            account = input("Ali imate račun (y za test)? (y/n): ")
            if account.lower() == "y":
                username = input("Ime (test): ")
                password = input("Geslo (1234): ")
...
def send_login(username, psw):
    ...
    if new_token != "!INCORRECT_PASSWORD" and new_token != "!FALSE_TOKEN":
        file = open("client/token/token.txt", "w")
...
def send(msg, ret):
    # Odpre datoteko
    server_public_pem_path = Path("client/keys/server_public_key.pem")
...
def rec(loop):
    ...
    # Preveri, če je prejšnja funkcija v prijavnem postopku ali v glavnem delu
    ret = True
    if loop:
        ret = False
    ...
    try:
        while True:
            ...
            try:
                ...
                # Drugače jo samo shranimo na seznam
...
def send_public_key():
    public_pem_bytes = Path("client/keys/public_key.pem").read_bytes()
    return send(public_pem_bytes.decode(FORMAT), True)


# Funkcija za branje privatnega ključa
def rprivate_key():
    private_pem_bytes = se.decryption(Path("client/keys/privat_key.pem").read_bytes().decode(FORMAT))
...
def main():
    ...
    while True:
        try:
            ...
            file = open("client/token/token.txt", "r+")
        ...
        # Če datoteke token.txt ni jo ustvari
            ...
            file = open("client/token/token.txt", "x")
            file.close()
